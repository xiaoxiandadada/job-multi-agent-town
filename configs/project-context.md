...
- 编排：默认 LangGraph 状态图为
  `route → context_phase → action_phase → judge`。阶段内并行受并发上限控制；
  另保留 `asyncio` baseline 用于性能对照和降级。
- 角色：7 个独立业务角色为岗位侦察员、JD 分析师、岗位知识补充员、
  简历策略师、作品教练、面试教练和证据审核员；另有 1 个飞书总控入口。
- 飞书：每个角色可绑定独立飞书自建应用，在同一群中被单独 `@`。模型通过用户
  自己的 OpenAI-compatible API 调用，不使用飞书 AI。
- RPG 小镇：角色拥有独立建筑、坐标、图标和日程；页面状态由真实
  `ActivityEvent` 驱动，展示 route/context/action/judge、handoff、状态和延迟，
  不是定时伪动画。
...
- 日报：`job-agent-push-daily` 可把同一日报按职责拆给不同角色机器人推送。
...
1. LangGraph 分阶段协作：`src/job_agent_harness/langgraph_orchestrator.py`，
   由 `tests/test_langgraph_orchestrator.py` 复核。
